...
{
    //load config file
    config_ = json::Value(json::Value::ValueType::TYPE_OBJECT);
...
void Config::LoadAddrs()
{
    ...
    {
        jaddrs = json::Value(json::Value::ValueType::TYPE_OBJECT);
...
void Config::LoadSaveCondition()
{
    ...
    {
        int index = 0;
        jsave_conds = json::Value(json::Value::ValueType::TYPE_OBJECT);
        for(auto iter : save_conds_)
        {
            json::Value arr(json::Value::ValueType::TYPE_ARRAY);
            arr.ArrayResize(2);
            arr[0] = iter.first;
            arr[1] = iter.first;
...
void Config::LoadThreadNums()
{
    ...
    json::Value thread_nums;
    if(false == config_.PairGet(key, &thread_nums))
        config_.PairAdd(key, thread_nums_);
    else
        thread_nums_ = static_cast<int>(thread_nums.get_int());
